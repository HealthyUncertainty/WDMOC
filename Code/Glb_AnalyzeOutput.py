...
class Analyze_Output:
    ...
    def EntitySurvival(self, entity):
        ...
        if entity.death_desc == 'Dead of Natural Causes' and entity.time_death == [self.maxdays,0]:
            LYG = self.maxLYG
            QALY = LYG*ent_util[0][1]
        else:
            ...
            for h in (1,len(ent_util)-1):
                ...
                while day < ent_util[h][0]:
                    LYG += (1/365)*(1+self.disc_rate)**(-day)
                    QALY += (Util/365)*(1+self.disc_rate)**(-day)
                    day +=1
